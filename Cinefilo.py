    ...
    for i in range(generos):
        ...
        while True:
            ...
            if opera != "F":
                opera = opera.split()
                if opera[0] == "I":
                    BD.Insert(int(opera[1]))
                if opera[0] == "N":
                    nodo = BD.Search(BD.get_root(), int(opera[1]))
                    print(BD.Nivel(nodo))
                if opera[0] == "L":
                    int_inicio = int(opera[1])
                    int_fim = int(opera[2])
                    inicio = BD.Search(BD.get_root(), int_inicio)
                    fim = BD.Search(BD.get_root(), int_fim)
                    lista = []
                    BD.Inorder(BD.get_root(), lista)
                    if lista != []:
                        filtred = filter(lambda x: x >= int_inicio and x <= int_fim, lista)
                        campo = ""
                        for i in list(filtred):
                            campo += str(i) + " "
                        print(campo)
                    else:
                        print("-1")
